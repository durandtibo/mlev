...
The `mlev` team takes security bugs seriously. We appreciate your efforts to responsibly disclose
your findings.
...
When using `mlev` in your projects:

1. **Keep dependencies updated**: Regularly update to the latest version of `mlev` and its
   dependencies
...
`mlev` is a library for comparing complex objects. When using it:
...
- **Third-party data structures**: When extending `mlev` to support custom types, ensure proper
