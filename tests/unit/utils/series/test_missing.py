...
###########################################
#     Tests for series_contains_missing   #
###########################################
...
def test_series_contains_missing_no_missing(series: pl.Series, missing_policy: str) -> None:
    assert not series_contains_missing(series, missing_policy=missing_policy)
...
def test_series_contains_missing_with_missing(series: pl.Series) -> None:
    assert series_contains_missing(series)


def test_series_contains_missing_raise() -> None:
    with pytest.raises(ValueError, match="col contains at least one missing value"):
        series_contains_missing(pl.Series("col", [1, 2, None, 4]), missing_policy="raise")


def test_series_contains_missing_raise_series_name() -> None:
    with pytest.raises(ValueError, match="my_series contains at least one missing value"):
        series_contains_missing(pl.Series("my_series", [1, 2, None, 4]), missing_policy="raise")
...
def test_series_contains_missing_nan_is_not_missing(series: pl.Series, missing_policy: str) -> None:
    assert not series_contains_missing(series, missing_policy=missing_policy)
...
def test_series_contains_missing_empty(series: pl.Series) -> None:
    assert not series_contains_missing(series)


def test_series_contains_missing_all_none() -> None:
    assert series_contains_missing(pl.Series("col", [None, None, None], dtype=pl.Int64))


def test_series_contains_missing_single_none() -> None:
    assert series_contains_missing(pl.Series("col", [None], dtype=pl.Int64))


def test_series_contains_missing_single_value() -> None:
    assert not series_contains_missing(pl.Series("col", [1]))
